  ...
  useEffect(() => {
    createClient()
      ...
      .then(({ data }) => {
        ...
        setGradsUltra(data.filter(esUltra).map((p: Record<string, unknown>) => ({
          sku:        p.sku as string,
          nombre:     p.nombre as string,
          grad:       String(p.nombre ?? '').replace(/^ULTRA\s*/i, '').trim(),
          precio:     Number(p.precio),
          stockBaja:  Number(p.stock_baja ?? 0),
          stockMayo:  Number(p.stock_mayo ?? 0),
          stockPlaza: Number(p.stock_plaza ?? 0),
        })))
        const items: CatItem[] = data.filter(p => !esUltra(p)).map((p, i) => ({
          id: 20000 + i,
          nombre: p.nombre as string,
        ...
        // Un solo "Ultra" que abre el selector de graduación
        const ultra = data.filter(esUltra)
        if (ultra.length > 0) {
          items.push({ id: 8500, nombre: 'ULTRA (lente de contacto)', categoria: 'Lentes de Contacto', precio: Number(ultra[0].precio), sku: 'ULTRA-LC', stock: 999 })
        }
        setCatalogoDB(items)
      })
  }, [])
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  return (
    <div className="space-y-4 max-w-6xl">
      ...
      {/* Selector de graduación para Ultra (lente de contacto) */}
      {pendingUltra && (() => {
        const stockDe = (g: typeof gradsUltra[number]) =>
