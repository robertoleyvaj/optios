    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">Gestión de accesos y permisos por rol</p>
        </div>
        <button onClick={() => { setEditando(null); setForm(formVacio()); setModal(true) }}
          className="flex items-center gap-2 bg-[#0B1A35] text-white px-4 py-2.5 rounded text-sm font-semibold hover:bg-[#0d2145] transition-all">
      ...
      <div className="grid grid-cols-4 gap-4">
        {(Object.entries(ROL_CONFIG) as [Rol, typeof ROL_CONFIG[Rol]][]).map(([rol, cfg]) => (
          <button key={rol} onClick={() => setRolDetalle(rolDetalle === rol ? null : rol)}
            className={`text-left bg-white rounded-lg shadow-sm border border-slate-100 p-4 hover:border-slate-300 transition-all ${rolDetalle === rol ? 'ring-2 ring-[#2BBFB3]/40' : ''}`}>
            <div className="flex items-center justify-between mb-2">
              <span className={`text-xs font-bold px-2 py-1 rounded ${cfg.bg} ${cfg.text}`}>{cfg.label}</span>
              <Shield className="w-4 h-4 text-slate-300" />
            </div>
            <p className="text-xs text-slate-400 mt-1">{cfg.permisos.length} módulos · {usuarios.filter(u => u.rol === rol).length} usuario{usuarios.filter(u => u.rol === rol).length !== 1 ? 's' : ''}</p>
            {rolDetalle === rol && (
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-100 overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100">
          <p className="text-sm font-semibold text-slate-700">{usuarios.length} usuarios registrados · {usuarios.filter(u => u.activo).length} activos</p>
        </div>
                ...
                <tr key={u.id} className="hover:bg-slate-50 transition-colors group">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-[#0B1A35] text-white flex items-center justify-center text-xs font-bold flex-shrink-0">
                  ...
                  <td className="px-5 py-4 text-sm text-slate-500">{u.sucursal}</td>
                  <td className="px-5 py-4 text-xs text-slate-400">{u.ultimoAcceso}</td>
                  <td className="px-5 py-4">
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
              <h2 className="text-base font-bold text-slate-800">{editando ? 'Editar usuario' : 'Nuevo usuario'}</h2>
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1.5">{editando ? 'Nueva contraseña' : 'Contraseña'}</label>
                  <div className="relative">
                    <input type={showPass ? 'text' : 'password'} value={form.password} onChange={e => f('password', e.target.value)}
                      className="w-full border border-slate-200 rounded px-3 pr-9 py-2.5 text-sm bg-slate-50 focus:outline-none focus:ring-2 focus:ring-[#2BBFB3]/30"
              ...
              {form.rol && (
                <div className={`rounded-lg p-3 text-xs space-y-1 ${ROL_CONFIG[form.rol].bg}`}>
                  ...
                  <p className="text-slate-500">{ROL_CONFIG[form.rol].permisos.join(' · ')}</p>
                </div>
              )}
            </div>
            <div className="px-6 pb-5 flex gap-3">
              <button onClick={() => setModal(false)}
                className="flex-1 py-2.5 border border-slate-200 text-slate-600 rounded text-sm font-semibold hover:bg-slate-50">Cancelar</button>
