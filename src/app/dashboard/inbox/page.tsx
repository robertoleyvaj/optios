      ...
      <div className="w-80 flex-shrink-0 bg-white border-r border-slate-100 flex flex-col">
        <div className="px-4 py-4 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {noLeidos > 0 && (
              <span className="bg-[#2BBFB3] text-white text-xs font-bold px-1.5 py-0.5 rounded-full">
                {noLeidos}
              </span>
            )}
          </div>
          <button
            ...
            className="flex items-center gap-1.5 bg-[#0B1A35] text-white text-xs font-medium px-3 py-1.5 rounded-lg hover:bg-[#1B3A6B] transition-colors"
          >
          ...
          {cargando ? (
            ...
            return (
              <button
                ...
                className={`w-full text-left px-4 py-3.5 hover:bg-slate-50 transition-colors ${seleccionado?.id === m.id ? 'bg-slate-50' : ''}`}
              >
                <div className="flex items-start justify-between gap-2 mb-1">
                  <div className="flex items-center gap-1.5 min-w-0">
                    {noLeido && <div className="w-1.5 h-1.5 rounded-full bg-[#2BBFB3] flex-shrink-0 mt-0.5" />}
                    <p className={`text-sm truncate ${noLeido ? 'font-semibold text-slate-900' : 'text-slate-600'}`}>
                      {esPropio ? `→ ${m.para_valor}` : m.de}
                    </p>
                  </div>
                  <span className="text-xs text-slate-400 flex-shrink-0">{tiempoRelativo(m.created_at)}</span>
                </div>
                <p className={`text-sm truncate ${noLeido ? 'font-medium text-slate-800' : 'text-slate-500'}`}>{m.asunto}</p>
                <p className="text-xs text-slate-400 truncate mt-0.5">{m.cuerpo}</p>
              </button>
            )
          })}
        </div>
        ...
        {seleccionado ? (
          <>
            <div className="bg-white border-b border-slate-100 px-6 py-4">
              <h2 className="text-lg font-semibold text-slate-900">{seleccionado.asunto}</h2>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              ...
              <div className="bg-white rounded-xl border border-slate-100 p-5">
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-7 h-7 rounded-full bg-[#0B1A35] flex items-center justify-center">
                    <span className="text-white text-xs font-bold">{seleccionado.de[0]}</span>
                  </div>
                  <span className="text-sm font-medium text-slate-700">{seleccionado.de}</span>
                </div>
                <p className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">{seleccionado.cuerpo}</p>
              </div>

              {/* Respuestas */}
              {seleccionado.respuestas?.map(r => (
                <div key={r.id} className={`rounded-xl border p-5 ${r.de === usuario.nombre ? 'bg-[#2BBFB3]/5 border-[#2BBFB3]/20 ml-6' : 'bg-white border-slate-100'}`}>
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full bg-slate-200 flex items-center justify-center">
                        <span className="text-slate-600 text-xs font-bold">{r.de[0]}</span>
                      </div>
                      <span className="text-sm font-medium text-slate-700">{r.de}</span>
                    </div>
                    <span className="text-xs text-slate-400">{tiempoRelativo(r.created_at)}</span>
                  </div>
                  <p className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">{r.cuerpo}</p>
                </div>
              ))}
            </div>
            ...
            <div className="bg-white border-t border-slate-100 px-6 py-4">
              <div className="flex gap-3">
                <textarea
                  ...
                  rows={2}
                  className="flex-1 border border-slate-200 rounded-xl px-4 py-2.5 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-[#2BBFB3]/30"
                />
                <button
                  onClick={enviarRespuesta}
                  disabled={!respuesta.trim()}
                  className="px-4 py-2 bg-[#0B1A35] text-white rounded-xl hover:bg-[#1B3A6B] disabled:opacity-40 transition-colors flex items-center gap-2 text-sm font-medium"
                >
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
            ...
            <p className="text-slate-400 text-sm">Selecciona un mensaje para leerlo</p>
          </div>
        )}
      </div>
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg border text-sm font-medium transition-colors ${compForm.para_tipo === 'sucursal' ? 'bg-[#0B1A35] text-white border-[#0B1A35]' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}
                >
                  <MapPin className="w-3.5 h-3.5" /> Sucursal
                </button>
                <button
                  onClick={() => setCompForm(f => ({ ...f, para_tipo: 'usuario', para_valor: '' }))}
                  className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg border text-sm font-medium transition-colors ${compForm.para_tipo === 'usuario' ? 'bg-[#0B1A35] text-white border-[#0B1A35]' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}
                >
              ...
              {compForm.para_tipo === 'sucursal' ? (
                <div className="grid grid-cols-3 gap-2">
                  {SUCURSALES.map(s => (
                    <button
                      ...
                      className={`py-2 px-3 rounded-lg border text-sm font-medium transition-colors ${compForm.para_valor === s ? 'bg-[#2BBFB3]/10 border-[#2BBFB3] text-[#2BBFB3]' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}
                    >
                      {s}
                    </button>
                  ))}
                ...
                <input
                  ...
                  className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#2BBFB3]/30"
                />
              )}
              ...
              <textarea
                ...
                rows={4}
                className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-[#2BBFB3]/30"
              />
            </div>
            <div className="flex gap-3 px-6 pb-5">
              <button onClick={() => setShowCompose(false)} className="flex-1 border border-slate-200 text-slate-600 rounded-xl py-2.5 text-sm hover:bg-slate-50 transition-colors">
                Cancelar
              </button>
              <button
                onClick={enviarMensaje}
                disabled={!compForm.para_valor || !compForm.asunto || !compForm.cuerpo}
                className="flex-1 bg-[#0B1A35] text-white rounded-xl py-2.5 text-sm font-medium hover:bg-[#1B3A6B] disabled:opacity-40 transition-colors flex items-center justify-center gap-2"
              >
