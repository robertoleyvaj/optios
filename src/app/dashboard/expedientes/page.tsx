...
function ExpedientesContent() {
  ...
  return (
    <div className="flex gap-5 h-[calc(100vh-140px)]">
      ...
      <div className="w-72 flex-shrink-0 bg-white rounded-lg shadow-sm border border-slate-100 flex flex-col overflow-hidden">
        <div className="p-4 border-b border-slate-100">
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-bold text-slate-700">Expedientes</span>
            <span className="text-xs text-slate-400">{pacientes.length} pacientes</span>
          </div>
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" />
            <input value={busqueda} onChange={e => setBusqueda(e.target.value)}
              className="w-full pl-8 pr-3 py-2 text-sm bg-slate-50 border border-slate-200 rounded focus:outline-none focus:ring-2 focus:ring-[#2BBFB3]/30"
          ...
          {filtrados.map(p => {
            ...
            return (
              <button key={p.id} onClick={() => setSeleccionado(p)}
                className={`w-full text-left px-4 py-3.5 transition-colors flex items-center gap-3 ${esSeleccionado ? 'bg-[#0B1A35]' : 'hover:bg-slate-50'}`}>
                <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${esSeleccionado ? 'bg-[#2BBFB3]/20 text-[#2BBFB3]' : 'bg-slate-100 text-slate-500'}`}>
                  {p.nombre[0]}{p.apellido[0]}
                </div>
                <div className="min-w-0 flex-1">
                  <p className={`text-sm font-semibold truncate ${esSeleccionado ? 'text-white' : 'text-slate-700'}`}>
                    {p.nombre} {p.apellido}
                  </p>
                  <p className={`text-xs truncate ${esSeleccionado ? 'text-white/50' : 'text-slate-400'}`}>
                    {rv2 ? `Última receta: ${rv2.fecha.split('-').slice(0,2).join('/')}` : 'Sin receta'}
                  </p>
                </div>
                <ChevronRight className={`w-3.5 h-3.5 flex-shrink-0 ${esSeleccionado ? 'text-white/40' : 'text-slate-300'}`} />
              </button>
            )
          })}
          {filtrados.length === 0 && (
            <div className="text-center py-10 text-sm text-slate-400">Sin resultados</div>
          )}
        </div>

        <div className="p-4 border-t border-slate-100">
          <button onClick={() => setModalPaciente(true)}
            className="w-full flex items-center justify-center gap-2 py-2.5 bg-[#0B1A35] text-white rounded text-sm font-semibold hover:bg-[#0d2145] transition-colors">
      ...
      {seleccionado ? (
        <div className="flex-1 flex flex-col gap-4 overflow-hidden">
          ...
          {/* Info del paciente */}
          <div className="bg-white rounded-lg shadow-sm border border-slate-100 p-5 flex-shrink-0">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-full bg-[#0B1A35] text-white flex items-center justify-center text-xl font-bold flex-shrink-0">
                  {seleccionado.nombre[0]}{seleccionado.apellido[0]}
                </div>
                <div>
                  <h2 className="text-lg font-bold text-slate-800">{seleccionado.nombre} {seleccionado.apellido}</h2>
                  <div className="flex items-center gap-4 mt-1 flex-wrap">
                    <span className="flex items-center gap-1.5 text-sm text-slate-500">
                      <Phone className="w-3.5 h-3.5" /> {seleccionado.telefono}
                    </span>
                    {seleccionado.email && (
                      <span className="text-sm text-slate-500">{seleccionado.email}</span>
                    )}
              ...
              <div className="flex items-center gap-2">
                <button className="flex items-center gap-1.5 px-3 py-2 border border-slate-200 rounded text-xs text-slate-500 hover:bg-slate-50 transition-colors">
            ...
            <div className="grid grid-cols-3 gap-3 mt-4 pt-4 border-t border-slate-100">
              <div className="text-center">
                <p className="text-2xl font-bold text-slate-800">{seleccionado.recetas.length}</p>
                <p className="text-xs text-slate-400 mt-0.5">Receta{seleccionado.recetas.length !== 1 ? 's' : ''}</p>
              </div>
              <div className="text-center border-x border-slate-100">
                <p className="text-2xl font-bold text-slate-800">{seleccionado.citas.length}</p>
                <p className="text-xs text-slate-400 mt-0.5">Citas</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold text-slate-800">
                  ${seleccionado.ventas.reduce((s, v) => s + v.total, 0).toLocaleString('es-MX')}
                </p>
                <p className="text-xs text-slate-400 mt-0.5">Total compras</p>
              </div>
            </div>
          </div>

          {/* Tabs */}
          <div className="bg-white rounded-lg shadow-sm border border-slate-100 flex-1 flex flex-col overflow-hidden">
            <div className="flex items-center border-b border-slate-100 px-2">
              {([
                ...
                <button key={key} onClick={() => setTabActiva(key)}
                  className={`flex items-center gap-1.5 px-4 py-3.5 text-sm font-medium border-b-2 transition-colors -mb-px ${tabActiva === key ? 'border-[#2BBFB3] text-[#0B1A35]' : 'border-transparent text-slate-400 hover:text-slate-600'}`}>
                  <Icon className="w-3.5 h-3.5" /> {label}
                </button>
              ))}
              {tabActiva === 'recetas' && (
                <button onClick={() => { setFormReceta(formVacioReceta()); setErroresReceta({}); setModalReceta(true) }}
                  className="ml-auto mr-3 flex items-center gap-1.5 px-3 py-2 bg-[#0B1A35] text-white rounded text-xs font-semibold hover:bg-[#0d2145] transition-colors">
              ...
              {tabActiva === 'recetas' && (
                <div className="space-y-4">
                  ...
                  {seleccionado.recetas.sort((a, b) => b.fecha.localeCompare(a.fecha)).map((r, i) => (
                    <div key={r.id} className={`rounded-lg border ${i === 0 ? 'border-[#2BBFB3]/40 bg-[#2BBFB3]/5' : 'border-slate-200 bg-white'}`}>
                      {/* Header receta */}
                      <div className="flex items-center justify-between px-4 py-3 border-b border-inherit">
                        <div className="flex items-center gap-3">
                          <div className={`text-xs font-bold px-2 py-1 rounded ${i === 0 ? 'bg-[#2BBFB3] text-white' : 'bg-slate-100 text-slate-500'}`}>
                            {i === 0 ? 'Vigente' : 'Anterior'}
                          </div>
                          <span className="text-sm font-semibold text-slate-700">{r.fecha}</span>
                          <span className="text-xs text-slate-400">· {r.tipo} · {r.optometrista}</span>
                        </div>
                            ...
                            <tbody className="divide-y divide-slate-100">
                              <tr>
                                <td className="py-2.5 pr-4">
                                  <span className="text-xs font-bold text-slate-500 bg-slate-100 px-2 py-1 rounded">OD</span>
                                </td>
                                <td className="text-center py-2.5 px-3 font-mono font-semibold text-slate-800">{r.od_esfera || '—'}</td>
                                <td className="text-center py-2.5 px-3 font-mono font-semibold text-slate-800">{r.od_cilindro || '—'}</td>
                                <td className="text-center py-2.5 px-3 font-mono font-semibold text-slate-800">{r.od_eje || '—'}°</td>
                                {(r.od_add || r.oi_add) && <td className="text-center py-2.5 px-3 font-mono font-semibold text-slate-800">{r.od_add || '—'}</td>}
                              </tr>
                              <tr>
                                <td className="py-2.5 pr-4">
                                  <span className="text-xs font-bold text-slate-500 bg-slate-100 px-2 py-1 rounded">OI</span>
                                </td>
                                <td className="text-center py-2.5 px-3 font-mono font-semibold text-slate-800">{r.oi_esfera || '—'}</td>
                                <td className="text-center py-2.5 px-3 font-mono font-semibold text-slate-800">{r.oi_cilindro || '—'}</td>
                                <td className="text-center py-2.5 px-3 font-mono font-semibold text-slate-800">{r.oi_eje || '—'}°</td>
                                {(r.od_add || r.oi_add) && <td className="text-center py-2.5 px-3 font-mono font-semibold text-slate-800">{r.oi_add || '—'}</td>}
                              </tr>
                            </tbody>
                          </table>
                        </div>
                        <div className="flex items-center gap-4 mt-3 pt-3 border-t border-slate-100">
                          <span className="text-xs text-slate-500"><span className="font-semibold text-slate-600">D.P.:</span> {r.dp} mm</span>
                          {r.observaciones && (
                            <span className="text-xs text-slate-500"><span className="font-semibold text-slate-600">Obs.:</span> {r.observaciones}</span>
                          )}
                        </div>
                      </div>
                    </div>
              ...
              {tabActiva === 'citas' && (
                <div className="space-y-2">
                  {seleccionado.citas.length === 0 && (
                    <div className="text-center py-16 text-slate-400 text-sm">Sin citas registradas.</div>
                  )}
                  {[...seleccionado.citas].sort((a, b) => b.fecha.localeCompare(a.fecha)).map((c, i) => (
                    <div key={i} className="flex items-center gap-4 px-4 py-3 rounded-lg border border-slate-100 hover:bg-slate-50 transition-colors">
                      <div className="w-10 h-10 rounded bg-slate-100 flex items-center justify-center flex-shrink-0">
                        <Clock className="w-4 h-4 text-slate-400" />
                      </div>
                      <div className="flex-1">
                        <p className="text-sm font-semibold text-slate-700">{c.tipo}</p>
                        <p className="text-xs text-slate-400">{c.sucursal} · {c.fecha}</p>
                      </div>
              ...
              {tabActiva === 'ventas' && (
                <div className="space-y-2">
                  {seleccionado.ventas.length === 0 && (
                    <div className="text-center py-16 text-slate-400 text-sm">Sin compras registradas.</div>
                  )}
                  {[...seleccionado.ventas].sort((a, b) => b.fecha.localeCompare(a.fecha)).map((v, i) => (
                    <div key={i} className="flex items-center gap-4 px-4 py-3 rounded-lg border border-slate-100 hover:bg-slate-50 transition-colors">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-semibold text-slate-700">{v.productos}</p>
                        <p className="text-xs text-slate-400">{v.folio} · {v.fecha}</p>
                      </div>
                      <span className="text-sm font-bold text-slate-800">${v.total.toLocaleString('es-MX')}</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">{seleccionado?.nombre} {seleccionado?.apellido}</p>
              </div>
              <button onClick={() => { setModalReceta(false); setErroresReceta({}) }}><X className="w-5 h-5 text-slate-400" /></button>
            </div>
            <div className="px-6 py-5 space-y-5">
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1.5">Optometrista</label>
              ...
              <div className="border border-slate-200 rounded-lg overflow-hidden">
                ...
                <table className="w-full text-sm">
                  <thead className="bg-slate-50 border-b border-slate-200">
                    <tr>
                      <th className="text-left text-xs text-slate-400 font-medium px-4 py-2.5 w-16"></th>
                      <th className="text-center text-xs text-slate-400 font-medium px-2 py-2.5">Esfera</th>
                      <th className="text-center text-xs text-slate-400 font-medium px-2 py-2.5">Cilindro</th>
                      <th className="text-center text-xs text-slate-400 font-medium px-2 py-2.5">Eje</th>
                      {(formReceta.tipo === 'Progresivo' || formReceta.tipo === 'Bifocal') && (
                        <th className="text-center text-xs text-slate-400 font-medium px-2 py-2.5">Adición</th>
                      )}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {[
                      ...
                      <tr key={row.label}>
                        <td className="px-4 py-3">
                          <span className="text-xs font-bold text-slate-600 bg-slate-100 px-2 py-1 rounded">{row.label}</span>
                        </td>
                          ...
                          return (
                            <td key={field} className="px-2 py-3">
                              <input
                                ...
                                className={`w-full text-center border rounded px-2 py-2 text-sm font-mono bg-slate-50 focus:outline-none focus:ring-2 focus:ring-[#2BBFB3]/30 ${hasError ? 'border-red-400 bg-red-50 ring-1 ring-red-300' : 'border-slate-200'}`}
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full border rounded px-3 py-2.5 text-sm bg-slate-50 focus:outline-none focus:ring-2 focus:ring-[#2BBFB3]/30 font-mono ${erroresReceta.dp ? 'border-red-400 bg-red-50' : 'border-slate-200'}`}
                    placeholder="62"
                  />
            ...
            <div className="px-6 pb-5 flex gap-3">
              ...
              <button onClick={guardarReceta}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-[#0B1A35] text-white rounded text-sm font-bold hover:bg-[#0d2145]">
      ...
      {modalPaciente && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
              <h2 className="text-base font-bold text-slate-800">Nuevo paciente</h2>
              <button onClick={() => setModalPaciente(false)}><X className="w-5 h-5 text-slate-400" /></button>
            </div>
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1.5">Sucursal principal</label>
                  <div className="relative">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-1.5">Notas médicas</label>
